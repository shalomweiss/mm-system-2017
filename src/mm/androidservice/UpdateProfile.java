		...
		try {
			...
			try {
				...
				if (originalUser instanceof Mentee) {
					...
					if (updatedMentee.getFirstName() == (null))
						updatedMentee.setFirstName(originalUser.getFirstName());
					if (updatedMentee.getLastName() == (null))
						updatedMentee.setLastName(originalUser.getLastName());
					if (updatedMentee.getPhoneNumber() == (null))
						updatedMentee.setPhoneNumber(originalUser.getPhoneNumber());
					if (updatedMentee.getGender() != originalUser.getGender())
						updatedMentee.setGender(originalUser.getGender());
					if (updatedMentee.getAddress() == (null))
						updatedMentee.setAddress(originalUser.getAddress());
					if (updatedMentee.getNote() == (null)) {
					...
					if (updatedMentee.getRemainingSemesters() <= 0)
						updatedMentee.setRemainingSemesters(((Mentee) originalUser).getRemainingSemesters());
					if (updatedMentee.getGraduationStatus() == (null))
						updatedMentee.setGraduationStatus(((Mentee) originalUser).getGraduationStatus());
					if (updatedMentee.getAcademiclnstitution() <= 0)
						updatedMentee.setAcademiclnstitution(((Mentee) originalUser).getAcademiclnstitution());
					if (updatedMentee.getAverage() <= 0)
						updatedMentee.setAverage(((Mentee) originalUser).getAverage());
					if (updatedMentee.getRemainingSemesters() <= 0)
						updatedMentee.setRemainingSemesters(((Mentee) originalUser).getRemainingSemesters());
					if (updatedMentee.getAcademicDicipline() == (null))
					...
					if (updatedMentor.getFirstName() == (null))
						updatedMentor.setFirstName(originalUser.getFirstName());
					if (updatedMentor.getLastName() == (null))
						updatedMentor.setLastName(originalUser.getLastName());
					if (updatedMentor.getPhoneNumber() == (null))
						updatedMentor.setPhoneNumber(originalUser.getPhoneNumber());
					if (updatedMentor.getGender() != originalUser.getGender())
						updatedMentor.setGender(originalUser.getGender());
					if (updatedMentor.getAddress() == (null))
					...
					if (updatedMentor.getExperience() == (null))
						updatedMentor.setExperience(((Mentor) originalUser).getExperience());
					if (updatedMentor.getRole() == (null))
						updatedMentor.setRole(((Mentor) originalUser).getRole());
					if (updatedMentor.getCompany() <= 0)
						updatedMentor.setCompany(((Mentor) originalUser).getCompany());
					if (updatedMentor.getVolunteering() == (null))
						updatedMentor.setVolunteering(((Mentor) originalUser).getVolunteering());
					if (updatedMentor.getWorkHistory() == (null))
						updatedMentor.setWorkHistory(((Mentor) originalUser).getWorkHistory());
			...
			if (ServerUtils.validateUserSession(id, token, iom.getDataAccess())) {
				try {
					...
					if (iom.getDataAccess().editUser(sendToDB)) {
						// success
