...
public class DataAccess implements DataInterface {
	...
	public User login(String email) throws SQLException {
		...
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectUserByEmail);
		...
		if (rs.next()) {
			...
			case 1: // Tsofen member
				...
				u = new TsofenT(
						rs.getInt(DataContract.UsersTable.COL_ID),
						...
						rs.getBoolean(DataContract.UsersTable.COL_ACTIVE),
						userType.TSOFEN);
				break;
			case 2:// Mentor
				logger.log(Level.INFO, "User type Mentor");
				PreparedStatement stm2 = c
						.prepareStatement(SQLStatements.selectMentorById);
			...
			case 3:// Mentee
				...
				PreparedStatement stm3 = c
						.prepareStatement(SQLStatements.selectMenteeById);
	...
	public boolean editUser(User user) throws SQLException {
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectUserById);
		...
		PreparedStatement stm2 = c
				.prepareStatement(SQLStatements.updateUserById);
		...
		if (user.getType() == userType.MENTOR) {
			PreparedStatement stm3 = c
					.prepareStatement(SQLStatements.updateMentorById);
		...
		if (user.getType() == userType.MENTEE) {
			PreparedStatement stm4 = c
					.prepareStatement(SQLStatements.updaterMenteeById);
	...
	public boolean deactivateUser(int id) throws SQLException {
		...
		if (rs.next()) {
			PreparedStatement stm2 = c
					.prepareStatement(SQLStatements.setUserDeactiveById);
			stm2.setInt(1, id);
			return true;
		}
		return false;
	...
	public int addUser(User u) throws SQLException {
		...
		if (u.getType() == userType.MENTOR) {
			...
			PreparedStatement stm3 = c
					.prepareStatement(SQLStatements.insertMentor);
			stm3.setInt(1, id);
		...
		if (u.getType() == userType.MENTEE) {
			PreparedStatement stm4 = c
					.prepareStatement(SQLStatements.insertMentee);
			stm4.setInt(1, id);
	...
	public ArrayList<User> getUsers(userType type) throws SQLException {
		...
		case TSOFEN:
			PreparedStatement stm = c
					.prepareStatement(SQLStatements.selectUserByType);
			stm.setInt(1, type.getValue());
			ResultSet r = stm.executeQuery();
			while (r.next()) {
				u = new TsofenT(
						r.getInt(DataContract.UsersTable.COL_ID),
						...
						r.getBoolean(DataContract.UsersTable.COL_ACTIVE),
						userType.TSOFEN);
			}
		...
		case MENTOR:
			...
			while (r2.next()) {
				u = new Mentor(
						r2.getInt(DataContract.UsersTable.COL_ID),
						...
						r2.getBoolean(DataContract.UsersTable.COL_ACTIVE),
						userType.MENTOR,
		...
		case MENTEE:
			...
			while (r3.next()) {
				u = new Mentee(
						r3.getInt(DataContract.UsersTable.COL_ID),
						...
						r3.getBoolean(DataContract.UsersTable.COL_ACTIVE),
						userType.MENTEE,
	...
	public boolean disconnectPair(int pairId) {
		try {
			PreparedStatement stm = c
					.prepareStatement(SQLStatements.setPairDeactiveById);
			stm.setInt(1, pairId);
			stm.executeUpdate();
	...
	public Pair getPair(int pairId) throws SQLException {
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectPairById);
		...
		return new Pair(rs.getInt(DataContract.PairsTable.COL_PAIRID),
				...
				rs.getInt(DataContract.PairsTable.COL_ACTIVESTATUS), rs
						.getDate(DataContract.PairsTable.COL_STARTDATE)
						.getTime(), l,
	...
	public ArrayList<Session> getUserSessions(int id) throws SQLException {
		...
		Session s = null;
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectSessionByUserId);

		stm.setInt(1, id);
		ResultSet rs = stm.executeQuery();
		while (rs.next()) {
			s = new Session(id,
					rs.getString(DataContract.SessionsTable.COL_TOKEN),
	...
	public ArrayList<Meeting> getUserMeetings(int id) throws SQLException {
		...
		Meeting meet = null;
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectMeetingsByMentorId);
		stm.setInt(1, id);
		ResultSet rs = stm.executeQuery();

		if (rs.next()) {
			java.sql.Date SDate1 = rs
					.getDate(DataContract.MeetingTable.COL_DATE);
			meet = new Meeting(
					...
					meetingStatus.valueOf(rs
							.getInt(DataContract.MeetingTable.COL_STATUS)),
					...
					meetingType.getByValue(rs
							.getInt(DataContract.MeetingTable.COL_MEETINGTYPE)),
			...
			PreparedStatement stm1 = c
					.prepareStatement(SQLStatements.selectMeetingsByMenteeId);
			stm1.setInt(1, id);
			ResultSet rs1 = stm.executeQuery();
			if (rs1.next()) {
				java.sql.Date SDate = rs
						.getDate(DataContract.MeetingTable.COL_DATE);
				meet = new Meeting(
						rs.getInt(DataContract.MeetingTable.COL_ACTIVITYID),
						rs.getInt(DataContract.MeetingTable.COL_PAIRID),
	...
	public Mentor getMentorOfMentee(int menteeId) throws SQLException {
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectPairsByMenteeIdAndActiveStats);
		stm.setInt(1, menteeId);
	...
	public ArrayList<Mentee> getMenteesOfMentor(int mentorId)
		...
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectPairsByMentorIdAndActiveStats);
		stm.setInt(1, mentorId);
		stm.setInt(2, 1);
		ResultSet rs = stm.executeQuery();
		while (rs.next()) {
	...
	public Meeting getMeetingById(int meetingId) throws SQLException {
		Meeting m = null;
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectMeetingById);
	...
	public boolean addMeeting(Meeting meeting) {
		try {
			String d = null;
			PreparedStatement stm = c
					.prepareStatement(SQLStatements.addMeeting);
	...
	public ArrayList<Meeting> getMeetingsByPairId(int pairId)
		...
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectMeetingByPairId);
		stm.setInt(1, pairId);
		ResultSet rs = stm.executeQuery();
	...
	public ArrayList<Mentee> getAllMenteesWithoutMentor() throws SQLException {
		...
		while (r.next()) {
			u = new Mentee(
					...
					r.getBoolean(DataContract.MenteeTable.COL_SIGNEDEULA), r
							.getString(DataContract.MenteeTable.COL_RESUME), r
							.getString(DataContract.MenteeTable.COL_GRADESHEET));
			menteesList.add(u);
		}
	...
	public boolean addWorkPlace(WorkPlace workplace) throws SQLException {
		...
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.insertWorkPlace);
	...
	public boolean addAcademicInstitute(AcademicInstitute a)
			throws SQLException {
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.insertAcademicinstitute);
	...
	public ArrayList<AcademicInstitute> getAllAcademiclnstitution()
		...
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectAcademicInstitute);
		ResultSet rs = stm.executeQuery();
		while (rs.next()) {
			academic = new AcademicInstitute(
					rs.getInt(DataContract.AcademicInstituteTable.COL_ID),
	...
	public ArrayList<WorkPlace> getAllWorkingPlace() throws SQLException {
		...
		WorkPlace w = null;
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectWorkPlace);
		ResultSet rs = stm.executeQuery();
		while (rs.next()) {
	...
	@Override
	public ArrayList<Mentee> getAllCorrespondingMentees(String address,
			int gender, int academicInstitution, int inPair,
	...
	@Override
	public ArrayList<Mentor> getAllCorrespondingMentors(String address,
			int gender, int workPlace, int inPair) throws SQLException {
	...
	@Override
	public ArrayList<Pair> getAllCorrespondingPairs(int numOfMeetings,
			String mentorFirstName, String mentorLastName, long startingAt,
			long endingAt, meetingType t) throws SQLException {
		ArrayList<Pair> pairs = new ArrayList<>();
		Pair p = null;
		java.sql.CallableStatement cStmt = c
				.prepareCall("{call getAllCorrespondingPairs(?, ?, ?, ?, ?, ?)}");
		...
		while (rs.next()) {
			p = new Pair(rs.getInt(DataContract.PairsTable.COL_PAIRID),
					rs.getInt(DataContract.PairsTable.COL_MENTORID),
					rs.getInt(DataContract.PairsTable.COL_MENTEEID),
					rs.getInt(DataContract.PairsTable.COL_ACTIVESTATUS),
					rs.getLong(DataContract.PairsTable.COL_STARTDATE),
					rs.getLong(DataContract.PairsTable.COL_ENDDATE),
					rs.getString(DataContract.PairsTable.COL_JOINTMESSAGE),
					rs.getString(DataContract.PairsTable.COL_TSOFENMESSAGE));
			pairs.add(p);
		}
	...
	public WorkPlace getWorkPlaceById(int id) throws SQLException {
		WorkPlace work = null;
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectWorkPlaceId);
		stm.setInt(1, id);
		ResultSet rs = stm.executeQuery();
		if (rs.next()) {
			work = new WorkPlace(id, rs.getString(2), rs.getString(3),
	...
	public AcademicInstitute getAcademicInstituteById(int id)
		...
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectAcademicInstituteId);
		stm.setInt(1, id);
		ResultSet rs = stm.executeQuery();
		if (rs.next()) {
			a = new AcademicInstitute(id, rs.getString(2), rs.getString(3),
					rs.getString(4));
		}
		return a;
	}
	...
	@Override
	public boolean changeMeetingStatus(int meetingId, int userId,
			meetingStatus status) throws SQLException {
		...
		case 4: // CANCELE
			PreparedStatement stm1 = c
					.prepareStatement(SQLStatements.updateStatus);
		...
		case 2:
			...
			PreparedStatement stm2 = c
					.prepareStatement(SQLStatements.completeMentor); // only one
																		// will
																		// work
			...
			stm2.executeUpdate();
			PreparedStatement stm3 = c
					.prepareStatement(SQLStatements.completeMentee); // only one
																		// will
																		// work
			...
			PreparedStatement stm4 = c
					.prepareStatement(SQLStatements.checkIfComplete);
			stm4.setInt(1, meetingId);
			ResultSet rs = stm4.executeQuery();
			if (rs.next()) {
				PreparedStatement stm5 = c
						.prepareStatement(SQLStatements.updateStatus);
	...
	public int getPairId(int menteeid, int mentorId) throws SQLException {
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.selectPairByMentorAndMenteeId);
		stm.setInt(1, mentorId);
		stm.setInt(2, menteeid);
	...
	@Override
	public boolean editMeetingNote(int meetingId, String message)
			throws SQLException {
		PreparedStatement stm = c
				.prepareStatement(SQLStatements.updateMeetingNote);
